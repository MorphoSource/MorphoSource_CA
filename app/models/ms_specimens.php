			...
			if($vs_uuid){
				...
				if($va_results["success"]){
					if(is_array($va_results["data"]["items"][0]) && sizeof($va_results["data"]["items"][0])){
						...
						$q_taxon_hits = $o_search->search(trim($va_specimen_info["indexTerms"]["genus"]." ".$va_specimen_info["indexTerms"]["specificepithet"]." ".$va_specimen_info["indexTerms"]["infraspecificepithet"])."*", array('sort' => 'ms_taxonomy_names.genus'));
