...
// 						break;
		...
		public function GetARK() {		
			...
			if ($t_media_file->get('ark')) {
				$va_errors["general"] = "this media file already has an ARK";
				return;
			}
			...
			$pn_media_id = $t_media_file->get("media_id");
			try {
				if ($vs_ark = $o_ark->createARK("M{$pn_media_file_id}", array(
					"datacite.creator" => trim($t_user->get('fname').' '.$t_user->get('lname')),
					"datacite.title" => "M{$pn_media_id}-{$pn_media_file_id}",
					"datacite.publisher" => "MorphoSource.org",
					"datacite.publicationyear" => date("Y"),
					...
					$this->notification->addNotification("Obtained ARK for media", __NOTIFICATION_TYPE_INFO__);
					# --- record the DOI in the DB
					$t_media_file->set("ark", $vs_ark);
					$t_media_file->setMode(ACCESS_WRITE);
					$t_media_file->update();
				} else {
					$this->notification->addNotification("Could not get ARK for media: ".$o_ark->getError(), __NOTIFICATION_TYPE_ERROR__);
				}	
			} catch (Exception $e) {
					$this->notification->addNotification("Could not get ARK for media: ".$e->getMessage(), __NOTIFICATION_TYPE_ERROR__);
			}
			$this->mediaInfo();
		}
 		# -------------------------------------------------------
