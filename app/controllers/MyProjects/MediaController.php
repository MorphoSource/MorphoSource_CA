...
// 						break;
 		...
 		# Begin JMW changes
 		public function batchMediaCheckTest() {
 			$va_media_ids = $this->request->getParameter('media_ids', pArray);
 			if(!is_array($va_media_ids)){
				$this->notification->addNotification("You did not select any media groups", __NOTIFICATION_TYPE_ERROR__);
			}else{
				if(is_array($va_media_ids) && sizeof($va_media_ids)){
					# Export all checked media IDs to console
					$output = $va_media_ids;
    				if (is_array($output)){
				...
				$this->notification->addNotification("Listed checked items", __NOTIFICATION_TYPE_ERROR__);
			}
			$this->listItems();
 		}
 		# End JMW changes
