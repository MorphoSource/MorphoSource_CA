 ...
 */ 
 ...
 require_once(__CA_MODELS_DIR__."/ms_media_sets.php");
	...
	# --------------------------------------------------------------------------------------------
	  ...
	  * 
	  */
	function addOwnedGroupToCartLink($o_request, $pn_media_id, $pn_user_id = null, $va_cart_media_file_ids = null, $va_options = array()) {
		$vs_link = "";
		if(!$pn_media_id){
			return false;
		}else{
			if(!is_array($va_cart_media_file_ids)){
				...
				$t_media_cart = new ms_media_sets();
 				$va_cart_media_file_ids = 
 					$t_media_cart->getCartMediaFileIdsForUser($pn_user_id);
			}
			$o_db = new Db();
			$q_media_files = $o_db->query("SELECT mf.media_file_id 
				FROM ms_media_files mf 
				INNER JOIN ms_media AS m ON m.media_id = mf.media_id 
				where mf.media_id = ?", $pn_media_id);
			if($q_media_files->numRows()){
				$vb_show_add_link = false;
				if(is_array($va_cart_media_file_ids) && 
					sizeof($va_cart_media_file_ids)){
					$va_media_file_ids = array();
					while($q_media_files->nextRow()){
						$va_media_file_ids[] = 
				...
				if(!$vb_show_add_link){
					$vs_link = "<a href='#' onClick='$(this).parent().load(\"".
						caNavUrl($o_request, '', 'MediaCart', 'RemoveGroup', 
							array('media_id' => $pn_media_id, 
								"class" => $vs_class)).
						"\"); return false;' class='".$vs_class."' title='".
						_t("Remove from media cart")."'>".
						_t("remove <i class='fa fa-shopping-cart'></i>")."</a>";
				}else{
					$vs_link = "<a href='#' onClick='$(this).parent().load(\"".
						caNavUrl($o_request, '', 'MediaCart', 'AddGroup', 
							array('media_id' => $pn_media_id, 
								"class" => $vs_class)).
						"\"); return false;' class='".$vs_class."' title='".
						_t("Add to media cart")."'>".
						_t("add <i class='fa fa-shopping-cart'></i>")."</a>";
				}
			}
			return $vs_link;
		}
