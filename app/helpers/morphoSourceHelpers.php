 ...
 require_once(__CA_MODELS_DIR__."/ms_media_sets.php");
	...
	function addOwnedGroupToCartLink($o_request, $pn_media_id, $pn_user_id = null, $va_cart_media_file_ids = null, $va_options = array()) {
		...
		if(!$pn_media_id){
			...
			$o_db = new Db();
			$q_media_files = $o_db->query("SELECT mf.media_file_id FROM ms_media_files mf INNER JOIN ms_media AS m ON m.media_id = mf.media_id where mf.media_id = ?", $pn_media_id);
			if($q_media_files->numRows()){
				$vb_show_add_link = false;
				if(is_array($va_cart_media_file_ids) && sizeof($va_cart_media_file_ids)){
					$va_media_file_ids = array();
