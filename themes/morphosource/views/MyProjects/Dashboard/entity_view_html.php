<?php
	...
	function formatNestGroupLabel($vs_taxon_level, $vs_taxon_term, $vs_entity_type, $va_entity, $request_class, $left_padding=null, $right_padding=null) {
		...
		if (($vs_entity_type == 'specimen') && 
			...
			if ($va_entity['no_link']) {
				print "<a href='".$request_class->request->getBaseUrlPath().
					"/index.php/MyProjects/Dashboard/specimenWithoutTaxonomy/specimens_group_by/".
					$vs_taxon_level."/taxon_id/".$vn_taxon_id.
					"' target='_blank' title='Open specimens in new tab'>
					<img src='".$request_class->request->getThemeUrlPath().
				...
				print "<a href='".$request_class->request->getBaseUrlPath().
					"/index.php/MyProjects/Dashboard/specimenByTaxonomy/specimens_group_by/".
					$vs_taxon_level."/taxon_id/".$vn_taxon_id.
					"' target='_blank' title='Open specimens in new tab'>
					<img src='".$request_class->request->getThemeUrlPath().
					"/graphics/morphosource/ic_open_in_new_black_18px.svg' 
	...
	if ($va_entity && $vn_count) {
		print "<div id='entityContainerTile'".
			($vs_entity_format != 'tile' ? " style='display: none;'" : "").">";
		if ($vb_entity_nest) {
			formatEntityNest($va_entity, $vs_specimens_group_by, 'tile', 
				$vs_entity_type, $request_class);
		} else {
			formatEntityGroup($va_entity, 'tile', $vs_entity_type, 
				$request_class);
		}
		print "</div><!-- end entityContainerTile -->";

		print "<div id='entityContainerList'".
			($vs_entity_format != 'list' ? " style='display: none;'" : "").">";
		if ($vb_entity_nest) {
			formatEntityNest($va_entity, $vs_specimens_group_by, 'list', 
				$vs_entity_type, $request_class);
		} else {
			formatListHeader($vs_entity_type);
			formatEntityGroup($va_entity, 'list', $vs_entity_type, 
				$request_class);
		}
